...
public class SQLHelper {
    ...
    public static int getCount(final String sql, final Connection connection,
		...
		if("oracle".equals(dbName)){
            ...
            if (sql.indexOf("UNION") != -1){
                ...
                String[] tmp_sql = removeOrders(sql).split("UNION");
                String tmp_sql1 = "SELECT count(1) AS cnt FROM (" + tmp_sql[0] + ") tmp1";
                String tmp_sql2 = "SELECT count(1) AS cnt FROM (" + tmp_sql[1] + ") tmp2";
                StringBuilder connect_sql = new StringBuilder();
                connect_sql.append(tmp_sql1).append(" UNION ").append(tmp_sql2);
                countSql = "select SUM(cnt) from (" + connect_sql.toString() + ") tmp_count";

            } else {
                //countSql = "select count(1) from (" + "select 1 " + removeSelect(removeOrders(sql)) + ") tmp_count";//去掉全字段
                //	        countSql = "select count(1) " + removeSelect(removeOrders(sql));
                countSql = "select count(1) from (" + removeOrders(sql) + ") tmp_count";//全字段查询
            }
    ...
    public static void main(String[] args){
        String sql = "aaaaaa  UNION ssdaadsad";
        System.out.println(sql.indexOf("UN2"));
    }
}
