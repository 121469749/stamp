...
<head>
    ...
    <script type="text/javascript">
        ...
        function exportExcel() {
            ...
            $("#searchForm").attr("action","${ctx}/dealer/count/city/exportExcel");
            $("#searchForm").submit();
            $("#searchForm").attr("action","${ctx}/dealer/count/city/list");
        }
...
<h3 style="width: 80%;margin: 10px auto">
    当前经销商：${currentDealer.companyName}(${currentDealer.area.name})-印章数量统计</h3>
<form:form id="searchForm" modelAttribute="dto" action="${ctx}/dealer/count/city/list"
           method="post"
...
<table class="table table-striped table-bordered" style="width: 93%;margin: 10px auto">
    ...
    <c:forEach items="${page.list}" var="makeCompany">
        <tr>
            ...
            <td>${makeCompany.companyCountVO.stampRecordCount}</td>
            <td>${makeCompany.companyCountVO.phyStampCount}</td>
            <td>${makeCompany.companyCountVO.eleStampCount}</td>
            <td>${makeCompany.companyCountVO.logoutStampCount}</td>
            <td>${makeCompany.companyCountVO.reportStampCount}</td>
