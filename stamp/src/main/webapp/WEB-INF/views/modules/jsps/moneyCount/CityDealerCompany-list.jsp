...
<head>
    ...
    <script type="text/javascript">
        ...
        function exportExcel() {
            $("#searchForm").attr("action","${ctx}/dealer/moneyCount/city/excelExport");
            $("#searchForm").submit();
            $("#searchForm").attr("action","${ctx}/dealer/moneyCount/city/list");
        }
...
<h3 style="width: 80%;margin: 10px auto">
    当前经销商-${currentMakeCompany.companyName}-${currentMakeCompany.area.name}-刻章收费统计</h3>
<form:form id="searchForm" modelAttribute="makeCompanyMoneyCountDTO" action="${ctx}/dealer/moneyCount/city/list"
           method="post"
    ...
    <sys:treeselect id="area" name="company.area.id" value="${makeCompanyMoneyCountDTO.company.area.id}"
                    labelName="company.area.name"
                    labelValue="${makeCompanyMoneyCountDTO.company.area.name}"
...
<table class="table table-striped table-bordered" style="width: 93%;margin: 10px auto">
    ...
    <c:forEach items="${page.list}" var="makeCompany">
        <tr>
            ...
            <td><div style="width:150px!important;">${makeCompany.compAddress}</div></td>
            <td>${makeCompany.area.name}</td>
            <td>${makeCompany.legalName}</td>
            <td>${makeCompany.legalPhone}</td>
            <td>${makeCompany.soleCode}</td>
            <td>${makeCompany.companyMoneyCountVO.phyStampCount}</td>
            <td><fmt:formatNumber value="${makeCompany.companyMoneyCountVO.phyStampCountMoney*0.01}" type="currency"
                                  pattern="#0.00"/></td>
            <td>${makeCompany.companyMoneyCountVO.eleStampCount}</td>
            <td><fmt:formatNumber value="${makeCompany.companyMoneyCountVO.eleStampCountMoney*0.01}" type="currency"
                                  pattern="#0.00"/></td>
                <%--<td width="200px">--%>
