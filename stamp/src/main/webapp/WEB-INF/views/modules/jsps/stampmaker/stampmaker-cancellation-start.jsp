...
<div class="table-double">
    ...
    <div class="table-right">
        <p> ${result == null ? "":  "上次注销申请审核结果为"} ${result == null ? "":  result.checkState.getValue()}
            <c:if test="${result != null}">
                <a href="${ctx}/makeStampPage/cancelfinish" target="_blank">查看上次申请的信息详情</a>
            </c:if>

        </p>
        <c:if test='${result == null || result.checkState.getValue() == "审核不通过"}'>
            <form:form id="mainform" modelAttribute="licenseApplyDTO" action="${ctx}/makeStampAction/submit"
                       method="post" class="form-horizontal" enctype="multipart/form-data">
                <input name="licence.workType" type="text" id="legalName" class="form-control hide" value="LOGOUT"/>
                <div class="mod" style="display: none">开办流程说明</div>
                <div class="mod">
                    <h3>注销申请</h3>
                    <p>注销申请理由：</p>
                    <form:textarea class="form-control" style="height:100px" path="licence.workReason" type="text"
                                   id="reason"></form:textarea>
                    <table class="table maintable table-striped table-bordered">
                        <tbody>
                        <tr>
                            <td>企业名称</td>
                            <td>
                                <input value="${company.companyName}" id="enterprise_name" type="text"
                                       class="specialtext" readonly/>
                            </td>
                            <td>经济性质</td>
                            <td colspan="3">

                                <form:select path="licence.busType" class="form-control">
                                    <form:options items="${fns:getDictList('busType')}" itemLabel="label"
                        ...
                        <tr>
                            <td>法人姓名</td>
                            <td>
                                <form:input path="licence.legalName" type="text" id="legalName2" class="form-control"
                                            placeholder="输入法人姓名"/>
                            </td>
                            <td>法人身份证号码</td>
                            <td>
                                <form:input path="licence.legalCertCode" type="text" id="mainperson-id"
                                            class="form-control" placeholder="输入法人身份证号"/>
                            </td>
                            <td>法人联系电话</td>
                            <td>
                                <form:input path="licence.legalPhone" type="number" id="mainperson-phone"
                                            class="form-control" placeholder="输入法人联系电话"/>
                            </td>
                        </tr>
                        <tr>
                            <td>治安负责人姓名</td>
                            <td>
                                <form:input path="licence.policeName" type="text" id="securityperson"
                                            class="form-control" placeholder="输入治安负责人姓名"/>
                            </td>
                            <td>负责人身份证号码</td>
                            <td>
                                <form:input path="licence.policeIdCode" type="text" id="securityperson-id"
                                            class="form-control" placeholder="输入负责人身份证号"/>
                            </td>
                            <td>负责人联系电话</td>
                            <td>
                                <form:input path="licence.policePhone" type="number" id="securityperson-phone"
                                            class="form-control" placeholder="输入治安负责人联系电话"/>
                            </td>
                        </tr>
                        <tr>
                            <td>企业总人数</td>
                            <td colspan="2">
                                <form:input path="licence.headCount" type="number" id="allpeople" class="form-control"
                                            placeholder="输入企业总人数"/>
                            </td>
                            <td>其中特业从业人数</td>
                            <td colspan="2">
                                <form:input path="licence.specialCount" type="number" id="specialpeople"
                                            class="form-control" placeholder="输入特业从业人数"/>
                            </td>
                        </tr>
                        <tr>
                            <td>企业统一码</td>
                            <td colspan="5">
                                <input value="${company.soleCode}" type="text" id="unifiedcode" class="specialtext"
                                       readonly/>
                            </td>
                        </tr>
                        </tbody>
                                ...
                                <td style="width: 370px;">
                                    <div class="box">
                                        <div class="clearfix" style="position:relative;">
                                            <input type="text" value="${dict.value}" name="fileType"
                                                   style="display: none">
                                            <input type="button" class="btn" value="选择文件">
                                            <input type="text" readonly style="max-width: 150px">
                                            <input class='upload' name="attachment" type='file'
                                                   accept="image/jpeg,image/png,image/bmp">
                                            <input style="display: none" class="btn btn-primary showimg"
                                                   data-toggle='modal' data-target='.bs-example-modal-lg' type="button"
                                                   onclick="showimg(this)" value="查看">
                                        </div>
                    ...
                    <button type="button" id="submitbtn2" class="btn btn-primary" style="width: 10%;margin-left: 40%">
                        提交
                    </button>
                </div>
